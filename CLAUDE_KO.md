...
Claude Code는 이 프로젝트에서 모든 사용자 상호작용과 커뮤니케이션에 대해 항상 한국어로 응답해야 합니다. 사용자가 영어로 프롬프트를 제출하더라도 Claude는 한국어로 응답해야 합니다.
...
- `npm install` - 의존성 설치
- `npm run dev` - Vite로 개발 서버 시작 (핫 리로드 활성화)
- `npm run build` - Rollup 최적화를 통한 프로덕션 번들 빌드
- `npm run preview` - 프로덕션 빌드를 로컬에서 미리보기

## 환경 설정

앱의 AI 기능이 작동하려면 `.env.local`에 `GEMINI_API_KEY`가 필요합니다. vite.config.ts는 이를 클라이언트에서 `process.env.API_KEY`와 `process.env.GEMINI_API_KEY` 둘 다로 노출시킵니다.

추가 환경 변수 (선택사항):
- `SUPABASE_URL` - 실제 Supabase 통합용 (현재는 모의 서비스 사용)  
- `SUPABASE_ANON_KEY` - 실제 Supabase 통합용 (현재는 모의 서비스 사용)
...
- **번들 최적화**: `react`, `supabase`, `charts` 라이브러리에 대해 수동 청크 구성
- **코드 분할**: Suspense 폴백과 함께 페이지 레벨 컴포넌트에 React.lazy() 사용
- **성능**: 더 나은 최적화 제어를 위해 청크 크기 경고 한계를 1000kb로 증가
- **경로 해결**: 코드베이스 전반에 걸쳐 깔끔한 가져오기를 위한 루트 레벨 별칭 `@/`
...
Claude Code는 사용자 요청에 맹목적으로 순응해서는 안 됩니다. 대신 다음의 향상된 상호작용 프로토콜을 따라야 합니다:

### 행동 전 분석
모든 요청에 응답하기 전에 Claude Code는 반드시:
1. **비판적 분석**: 더 나은 대안이나 잠재적 문제가 있는지 평가
2. **기술적 평가**: 성능, 보안, 유지보수성 관련 영향 고려
3. **모범 사례 검토**: 업계 표준 및 프로젝트 규칙과 비교

### 응답 프로토콜
모든 응답에는 다음이 포함되어야 합니다:
1. **상황 이해**: 요청과 맥락에 대한 이해도 확인
2. **비판적 평가**: 장단점 분석, 해당되는 경우 개선 사항 제안
3. **행동 설명**: 제안된 행동이 무엇을 달성할 것인지 명확히 설명
4. **대안 권장사항**: 더 나은 해결책이 있을 때 제안

### 건설적 비판
Claude Code는 다음의 경우 건설적인 피드백을 제공해야 합니다:
- 요청된 접근 방식에 기술적 결함이나 보안 위험이 있을 때
- 더 나은 아키텍처 패턴이나 솔루션이 존재할 때
- 코드 품질이나 유지보수성을 개선할 수 있을 때
- 업계 모범 사례를 따르지 않고 있을 때

이는 더 높은 품질의 기술적 토론을 보장하고 차선책 구현을 방지합니다.

## 중요한 지침

**파일 유지보수 정책:**
- 새 파일을 만드는 것보다 기존 파일 편집을 항상 선호
- 목적에 절대적으로 필요한 경우가 아니면 절대로 파일을 만들지 않음
- 명시적으로 요청받지 않는 한 문서 파일(*.md)이나 README 파일을 능동적으로 만들지 않음
- 사용자가 명시적으로 요청한 경우에만 문서 파일 생성

**작업 완료 프로토콜:**
- 요청받은 것을 수행; 그 이상도 이하도 아님
- 불필요한 기능이나 문서 추가 없이 특정 작업에 집중